...
use App\Models\PersonalPosSimple;
...
class SimpleLoginController extends Controller
{
    public function __invoke(Request $request)
    {
        try {
            ...
            // Buscar usuario usando el modelo simplificado
            $user = PersonalPosSimple::where('email', $request->email)
            ...
            $userWithRelations = PersonalPosSimple::with(['sucursal', 'role'])
                ->where('id', $user->id)
                ->first();
            ...
            Log::error('SimpleLoginController error', [
